...
function renderAlerts(scan) {
  ...
  if (severity.error) {
    appendSignal(
      elements.alertList,
      'error',
      `${formatNumber(severity.error)} critical findings`,
      'Click to drill into critical files and lines.',
      { kind: 'severity', value: 'error', label: 'Critical findings' }
    );
  }
  if (debuggerCount) {
    appendSignal(
  ...
  if (largeFiles || longFunctions) {
    appendSignal(
      elements.alertList,
      'warning',
      `${formatNumber(largeFiles + longFunctions)} complexity hotspots`,
      'Large files and long functions may slow delivery.',
      { kind: 'types', value: ['large_file', 'long_function'], label: 'Complexity hotspots' }
    );
  }
  if (Number(readiness.percentage) < 60) {
    appendSignal(
      elements.alertList,
      'error',
      readiness.status || 'Release not ready',
      `${formatNumber(readiness.percentage)}% readiness needs leadership attention.`
    );
  }
